...
V1 的目标不是“预先准备所有游戏系统”，而是让新项目能稳定启动、由服务器安全处理数据、让客户端可显示结果，并让开发期可以排错和重置测试档案。

任何不满足这个目标的能力，先留在具体游戏项目或候选记录中，不进入 V1。
...
| 不加入的内容 | 以后归属 / 进入条件 |
|---|---|
| 商店、背包、装备、鞋子、翅膀、跑步机 | 具体游戏 Service；不能进入 V1。 |
| `SetSpeed`、`SetWins`、赠送物品 | 具体游戏开发面板；框架只保留整档安全重置。 |
...
V1 现在进入“只修复真实问题，不预建设施”的阶段。下一次修改必须先在候选记录补齐真实证据、范围、验收方式和不做项。
