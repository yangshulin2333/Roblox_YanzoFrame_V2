...
--[[
	_logger	保存 Logger
	_rootFolder	保存 YanzoFrame_V2_Remotes 文件夹
...
function NetService:Init(context)
	...
	local serverToClient = getOrCreateFolder(root, RemoteNames.ServerToClientFolder) --创建一个 ServerToClient 文件夹在 YanzoFrame_V2_Remotes 下面

	--全局变量
	self._rootFolder = root --引用类型，指向 YanzoFrame_V2_Remotes
	...
	NetService:Init(context) 完成后，Roblox 里应该有：
		...
		NetService._rootFolder = YanzoFrame_V2_Remotes
